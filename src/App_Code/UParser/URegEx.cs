...
namespace Pimp.UParser
{
    ...
    public static class URegEx
    {
        ...
        /// </summary>
        public static string _findProvinceLineValidation = @"[ \(M\)\*\^]+?\s+(Orc|Gnome|Dark Elf|Elf|Human|Avian|Dwarf|Faery|Halfling|Undead)\s+([\d{1,5}]+\sacres|-|DEAD)\s+(\d*,\d*,\d*gc|\d*,\d*gc|\d*gc|-|DEAD)\s+(\d+gc)\s+((Baroness|Baron|Countess|Count|King|Knight|Noble Lady|Lady|Lord|Marquis|Duke|Duchess|Princess|Prince|Peasant|Queen|Viscountess|Viscount|DEAD)(\s\d+)?)";
        ...
        public static Regex _findKingdomProvinceLines = new Regex(@"(-\sUnclaimed\s-|-\sAwaiting Activation\s-|[a-zA-Z\d\-\!'_]['a-zA-Z\d\-\! _]{0,30})(\(M\))?(\^)?(\+)?(\*)?\s+(Orc|Gnome|Dark Elf|Elf|Human|Avian|Dwarf|Faery|Halfling|Undead)\s+([\d{1,5}]+\sacres|-|DEAD)\s+(\d*,\d*,\d*gc|\d*,\d*gc|\d*gc|-|DEAD)\s+(\d+gc)\s+((Baroness|Baron|Countess|Count|King|Knight|Noble Lady|Lady|Lord|Marquis|Duke|Duchess|Princess|Prince|Peasant|Queen|Viscountess|Viscount|DEAD)(\s\d+)?)", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        public static Regex _findKingdomProvinceNames = new Regex(@"(-\sUnclaimed\s-|-\sAwaiting Activation\s-|[a-zA-Z\d\-\!'_]['a-zA-Z\d\-\! _]{0,30})(\(M\))?(\^)?(\+)?(\*)?\s+(Orc|Gnome|Dark Elf|Elf|Human|Avian|Dwarf|Faery|Halfling|Undead)\s+\d", RegexOptions.IgnoreCase | RegexOptions.Compiled);
        public static Regex _findKingdomProvinceNamesRemove = new Regex(@"(\(M\))?(\^)?(\+)?(\*)?\s+(Orc|Gnome|Dark Elf|Elf|Human|Avian|Dwarf|Faery|Halfling|Undead)\s+\d", RegexOptions.Compiled | RegexOptions.IgnoreCase);
