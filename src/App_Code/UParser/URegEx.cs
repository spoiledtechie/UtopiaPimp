...
namespace Pimp.UParser
{
    ...
    public static class URegEx
    {
        ...
        public static Regex _findKingdomProvinceLines = new Regex(@"(\d{0,2}\s+)(-\sUnclaimed\s-|-\sAwaiting Activation\s-|[a-zA-Z\d\-\!'_]['a-zA-Z\d\-\! _]{0,30})(\(M\))?(\(S\))?(\^)?(\+)?(\*)?\s+(Orc|Gnome|Dark Elf|Elf|Human|Avian|Dwarf|Faery|Halfling|Undead)\s+([\d{1,5}]+\sacres|-|DEAD)\s+(\d*,\d*,\d*gc|\d*,\d*gc|\d*gc|-|DEAD)\s+(\d+gc)\s+((Baroness|Baron|Countess|Count|King|Knight|Noble Lady|Lady|Lord|Marquis|Duke|Duchess|Princess|Prince|Peasant|Queen|Viscountess|Viscount|DEAD)(\s\d+)?)", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        public static Regex _findLineNumber = new Regex(@"^\s*\d{0,2}\s+", RegexOptions.IgnoreCase | RegexOptions.Compiled);
